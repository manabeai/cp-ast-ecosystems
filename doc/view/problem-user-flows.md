...
1. Constraint ペインの draft `? <= N <= ?` をクリック → 制約編集 UI が開く
2. 下限 `?` をクリック → 値ピッカー表示 → `1` を自由入力 → Enter
3. 上限 `?` をクリック → 値ピッカー表示 → `10` を自由入力 → Enter
4. 上限の `10` をクリック → 関数適用ポップアップ → `^`（power）を選択 → `6` を入力 → Enter
5. 確定
...
1. draft `? <= A_i <= ?` をクリック
2. 下限: `1` を自由入力
3. 上限: `10` を自由入力 → `10` をクリック → `^` → `9` → Enter
4. 確定
...
1. draft `? <= H <= ?` をクリック → 下限 `1` 自由入力 → 上限 `500` 自由入力 → 確定
2. draft `? <= W <= ?` をクリック → 下限 `1` 自由入力 → 上限 `500` 自由入力 → 確定
...
2. `辺リスト` テンプレートを選択
3. 本数（count）フィールドをクリック → 利用可能な変数一覧が表示される（ここでは N のみ）
4. `N` をクリック → count に N が入る
5. 式中の `N` をクリック → 関数適用ポップアップが開く（min, max, +, -, /, * など）
6. `-` をクリック → ポップアップ右半分に operand の選択肢（自由入力可）が表示される
7. `1` と入力 → Enter で確定 → count が `N - 1` になる
8. 確定
...
1. draft `? <= N <= ?` をクリック
2. 下限: `2` 自由入力
3. 上限: `2` 自由入力 → `2` をクリック → `*`（multiply） → `100000` 入力 → Enter
4. 確定
...
1. N range: 下限 `1` 自由入力、上限 `2` 自由入力 → `2` に `*` 適用 → `100000` → Enter
2. Q range: 同上（`1` 〜 `2 * 10^5`）
3. a range: 下限 `1` 自由入力、上限は変数一覧から `N` を選択
4. b range: 同上（`1` 〜 `N`）
...
2. 対象変数: `N`
3. 上界エリアをクリック → `2` 自由入力 → `2` に `*` 適用 → `100000` → Enter
4. 確定
...
1. T range: 下限 `1` 自由入力、上限 `10` 自由入力 → `10` に `^` 適用 → `5` → Enter
2. N range: 下限 `1` 自由入力、上限 `2` 自由入力 → `2` に `*` 適用 → `100000` → Enter
3. A_i range: 下限 `1` 自由入力、上限 `10` 自由入力 → `10` に `^` 適用 → `9` → Enter
