...
>
> **[Rev.1]** Sprint 3 Synthesis により改訂。S-1〜S-3, M-1, M-4, M-5, Coverage Gap B/F, Sample Gen 3.1 対応。
...
**[Rev.1]** S-1 対応: `typ`/`element_type`/`separator` を NodeKind から除去。型情報は ConstraintAST の `TypeDecl` に一元化、区切り文字は `RenderHint` 制約に移動。M-4 対応: Tuple.elements を `Vec<NodeId>` に変更。M-5 / Coverage Gap B 対応: `Choice` NodeKind を追加。L-1 対応: Hole から `hole_id` を除去。

| # | NodeKind | 役割 | Slot | 対応する競プロ概念 | 具体例 |
|---|----------|------|------|--------------------|--------|
| 1 | **Scalar** | 単一変数の宣言 | `name: Ident` | N, M, Q, K 等のパラメータ変数 | `N`（整数スカラー）, `S`（文字列スカラー） |
| 2 | **Array** | 1次元列の宣言 | `name: Ident`, `length: Reference` | A_1 A_2 … A_N | `A`（長さ N の整数配列） |
| 3 | **Matrix** | 2次元配列の宣言 | `name: Ident`, `rows: Reference`, `cols: Reference` | H×W グリッド, コスト行列 | `C[i][j]`（文字グリッド）, `A_{i,j}`（整数行列） |
| 4 | **Tuple** | 同一行で読む複数変数の組 | `elements: Vec<NodeId>` | ヘッダ行 `N M K S T X`, 辺 `u_i v_i w_i` | `(N, A, B)`（abc300_a 1行目）, `(u_i, v_i)`（辺リストの1行） |
...
| 8 | **Choice** | タグ依存の分岐構造 | `tag: Reference`, `variants: Vec<(Literal, Vec<NodeId>)>` | クエリ種別分岐 `1 x y` / `2 x` | abc278_d の 3 種クエリ |
| 9 | **Hole** | 未完成位置のプレースホルダ | `expected_kind: Option<NodeKindHint>` | 編集途中の空欄 | 「ここに配列定義が入る予定」 |

### 補足: NodeKind の設計判断

- **Scalar / Array / Matrix** を分離した理由: サイト調査で「スカラー」「1D配列」「2Dグリッド」が構造的に異なる読み方を要求することが確認された（空白区切り / 無区切り文字列 / 行列展開）。
- **[Rev.1]** 型情報（`ExpectedType`）と区切り文字（`Separator`）は NodeKind から除去した。これらは ConstraintAST の `TypeDecl` および `RenderHint` として管理する。StructureAST は純粋に「何がどこにあるか」の構造のみを表現する（S-1 対応）。
- **[Rev.1] Tuple.elements を `Vec<NodeId>` に変更**: Tuple が子ノード（Scalar 等）を直接保持する。これにより Tuple の各要素が木構造上の親子関係を正しく形成する（M-4 対応）。
- **Repeat** は「M行の辺リスト」「T個のテストケース」「Q個のクエリ」をすべて統一的に表現する。body の内容が Tuple なら辺リスト、Section なら複数テストケースになる。
- **[Rev.1] Choice** はクエリ種別タグ（先頭整数）に応じて行構造が変わるパターンを表現する（M-5 / Coverage Gap B 対応）。tag は型コード変数への参照、variants は (型コード値, body ノード列) のペア。
- **Section** は abc299_e（グラフ+条件）や abc371_c（2グラフ+コスト行列）のような複合入力を表現する。
- **[Rev.1] Hole** は Hazelnut に倣い第一級ノードとして扱う。`hole_id` フィールドは `StructureNode.id` と冗長であるため除去した（L-1 対応）。Hole への制約付与は許可する（ConstraintAST が NodeId で管理、L-4 対応）。
...
**[Rev.1]** S-1 対応: `TypeDecl` が型情報の唯一の情報源に昇格。Coverage Gap F / Sample Gen 3.1 対応: `CharSet`, `StringLength` 制約を追加。`RenderHint` を追加（separator 等のレンダリング情報を管理）。

| # | Constraint | 何を制約するか | 対象 NodeKind | 具体例 |
|---|-----------|---------------|---------------|--------|
| 1 | **Range** | 変数の値域（下界・上界） | Scalar, Array(要素), Matrix(要素) | `1 ≤ N ≤ 2×10^5`, `0 ≤ A_i ≤ 10^9` |
| 2 | **TypeDecl** | 変数の型宣言 | Scalar, Array, Matrix | `N は整数`, `S は文字列`, `C[i][j] は文字` |
| 3 | **LengthRelation** | 配列長・繰り返し数と変数の関係 | Array, Matrix, Repeat, Scalar(Str) | `len(A) = N`, `辺数 = M`, `len(S) = L` |
...
| 10 | **CharSet** | 文字列/文字変数の使用文字集合 | Scalar(Str/Char), Array(Char要素), Matrix(Char要素) | `英小文字のみ`, `{o, x, .} のみ` |
| 11 | **StringLength** | 文字列の長さ制約 | Scalar(Str) | `1 ≤ |S| ≤ 1000` |
| 12 | **RenderHint** | 表示時の区切り・フォーマット指定 | Matrix, Array | `separator=Space`, `separator=None` |

### 補足: Constraint の設計判断

- **Range** が最も頻出であり、サイト調査 14問中 13問で出現。下界・上界は式（Expression）で表現し、`N(N-1)/2` のような導出上界にも対応する。
- **[Rev.1] TypeDecl** は型情報の唯一の情報源（Single Source of Truth）である。Sprint 2 では NodeKind にも `typ`/`element_type` があり型情報が二重管理されていたが、S-1 対応で ConstraintAST に一元化した。
- **LengthRelation** は StructureAST の Repeat.count / Array.length と ConstraintAST を橋渡しする重要な制約。**[Rev.1]** 文字列の長さ制約にも適用可能とする（`target` が Scalar(Str) の場合）。
...
- **[Rev.1] CharSet** は文字列生成に必須。文字集合なしでは Str/Char 型の変数を正しく生成できない（Sample Gen 3.1 対応）。
- **[Rev.1] StringLength** は文字列長を `min ≤ |S| ≤ max` 形式で制約する。Range は数値変数向けであるため、文字列長には専用制約を設ける（Coverage Gap F 対応）。
- **[Rev.1] RenderHint** は StructureAST から除去した `Separator` 等のレンダリング情報を管理する。構造と表示の関心を分離する（S-1 対応）。
...
| Slot 種別 | 意味 | 例 |
|-----------|------|-----|
| **Single** | 単一の子ノードまたは値を保持 | Array.length → Reference(N), Choice.tag → Reference |
| **List** | 順序付きの子ノードリストを保持 | Sequence.children → Vec\<NodeId\>, Tuple.elements → Vec\<NodeId\> |
...
2. **[Rev.1] Hole は制約を受け付ける**: Hole の NodeId に対して ConstraintAST で制約を紐づけることができる。FillHole 時に Hole に付いていた制約は新ノードに引き継がれる（L-4 対応）。
3. **Hole は差し替え可能**: Hole を具体ノードに置き換える操作は Operation 層が担う。
4. **Hole は入れ子可能**: Hole を含むノード（例: body に Hole を含む Repeat）は合法な状態である。

**[Rev.1] HoleId の廃止（L-1 対応）:**
- Hole は StructureNode.id で識別する。従来の `hole_id: HoleId` フィールドは StructureNode.id と冗長であるため除去した。
- ConstraintAST は StructureNode.id で Hole を参照する。
...
| Matrix(H×W) + RenderHint(sep=None) | H行、各行 W 文字連結 | 文字グリッド |
| Matrix(H×W) + RenderHint(sep=Space) | H行、各行 W 要素空白区切り | 整数行列 |
...
| **TypeDecl** | 型に応じた生成器を選択 | Int → 整数, Str → 文字列（CharSet 参照）, Char → 文字（CharSet 参照） |
| **LengthRelation** | 先に長さ変数を生成し、その値で配列長を決定 | 依存グラフの順序に従う。文字列長にも適用可能 |
...
| **CharSet** | 指定文字集合からランダムにサンプリング | **[Rev.1]** 文字列/文字の生成に必須 |
| **StringLength** | 下界〜上界の長さを先に決定、その長さで文字列生成 | **[Rev.1]** Range の文字列長版 |
| **RenderHint** | 生成には影響しない（rendering 層のみ） | **[Rev.1]** 区切り文字等のフォーマット指定 |
...
> 4. **[Rev.1] 制約の相互作用が充足可能**: Distinct + Range → 値域幅 ≥ 配列長、Property(Connected) + Range(M) → M ≥ N-1 等（M-3 対応）
> 5. **[Rev.1] Hole 非存在**: StructureAST に Hole が残っていない

**[Rev.1] 3 段階保証レベル（Sample Gen 4.4 より導入）:**

| レベル | 名称 | 意味 | 条件 |
|--------|------|------|------|
| **L1** | `Guaranteed` | 確実に生成可能 | Range + LengthRelation + TypeDecl(Int) のみで構成 |
| **L2** | `HighProbability` | 高確率で生成可能 | L1 + Distinct, Sorted, Relation(単純), Property(対応済み) |
...
- **[Rev.1]** 制約相互作用の非充足 → エラー: "constraint interaction unsatisfiable" (例: Distinct 要素数 > 値域幅)

---

## 6. Rust 型定義ドラフト

**[Rev.1]** S-1, S-2, S-3, M-1, M-4, M-5, L-1, Coverage Gap B/F, Sample Gen 3.1 対応の改訂。
...
/// [Rev.1] 制約の一意識別子（S-2 対応: RemoveConstraint に必要）
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
/// [Rev.1] M-1 対応: HashMap → Arena (Vec) に変更。
/// 挿入順序が保持され、canonical rendering の決定論的順序が保証される。
...
/// [Rev.1] ノード種別 — 純粋な構造のみ。型・区切り情報は ConstraintAST に移動（S-1 対応）。
/// Choice を追加（M-5 / Coverage Gap B 対応）。
#[derive(Debug, Clone, PartialEq)]
pub enum NodeKind {
    /// 単一変数: N, M, S 等
    /// [Rev.1] typ フィールド除去 — 型は TypeDecl 制約で管理
    Scalar {
        name: Ident,
    },
    /// 1次元配列: A_1 ... A_N
    /// [Rev.1] element_type フィールド除去
    Array {
        name: Ident,
        length: Reference,
    },
    /// 2次元配列 / グリッド: C[i][j], A_{i,j}
    /// [Rev.1] element_type, separator フィールド除去
    Matrix {
        name: Ident,
        rows: Reference,
        cols: Reference,
    ...
    /// [Rev.1] elements を Vec<NodeId> に変更 — Tuple が子ノードを直接保持（M-4 対応）
    Tuple {
        elements: Vec<NodeId>,
    },
    ...
    /// [Rev.1] タグ依存の分岐構造（M-5 / Coverage Gap B 対応）
    /// クエリ種別 (type_code) に応じて行構造が変わるパターンを表現する。
    Choice {
        tag: Reference,
        variants: Vec<(Literal, Vec<NodeId>)>,
    },
    /// [Rev.1] 未完成位置（第一級）— hole_id 除去（L-1 対応）
    Hole {
        expected_kind: Option<NodeKindHint>,
    },
}

/// [Rev.1] ExpectedType は ConstraintAST の TypeDecl 専用に移動。
/// StructureAST の NodeKind からは除去済み。
...
/// [Rev.1] Separator は ConstraintAST の RenderHint に移動。
/// NodeKind からは除去済み。
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Separator {
    Space,
...
/// [Rev.1] S-2 対応: ConstraintId の導入と ConstraintSet の再設計。
/// 制約は ConstraintId で一意に識別され、RemoveConstraint で正確に指定できる。
#[derive(Debug, Clone, PartialEq)]
pub struct ConstraintSet {
    /// 制約の実体（ConstraintId → Constraint のマッピング）
    ...
    /// 次に割り当てる ConstraintId
    pub next_id: u64,
}

/// 制約種別
/// [Rev.1] CharSet, StringLength, RenderHint を追加。
#[derive(Debug, Clone, PartialEq)]
pub enum Constraint {
    ...
    /// 型宣言: 変数/入力全体の型
    /// [Rev.1] S-1 対応: TypeDecl が型情報の唯一の情報源
    TypeDecl {
        target: Reference,
        expected: ExpectedType,
    },
    /// 長さ関係: len(array_or_str) = expr
    /// [Rev.1] length を Expression に拡張し、文字列にも適用可能とした
    LengthRelation {
        target: Reference,
        length: Expression,
    },
    ...
    /// [Rev.1] 文字集合制約（Sample Gen 3.1 / Coverage Gap F 対応）
    CharSet {
        target: Reference,
        charset: CharSetSpec,
    },
    /// [Rev.1] 文字列長制約（Coverage Gap F 対応）
    StringLength {
        target: Reference,
        min: Expression,
        max: Expression,
    },
    /// [Rev.1] レンダリングヒント（S-1 対応: separator を StructureAST から移動）
    RenderHint {
        target: Reference,
        hint: RenderHintKind,
    },
...
/// [Rev.1] 文字集合の定義（Sample Gen 3.1 対応）
#[derive(Debug, Clone, PartialEq, Eq)]
...
## [Rev.1] 延期事項（Known Limitations）

Sprint 3 レビューで検出されたが、Phase 1 では対処せず Phase 2 以降に延期する項目:

| # | 項目 | 出典 | 延期理由 |
|---|------|------|----------|
| D-1 | 三角行列 NodeKind (`TriangularMatrix` or `MatrixShape`) | Coverage Gap D | 出現頻度低。Repeat + 可変長 Tuple で暫定対応可能 |
| D-2 | 文字列パターン/正規表現制約 (`Pattern`) | Coverage Gap E | 出現頻度低。Property(Custom("regex:...")) で近似可能 |
| D-3 | BigInt リテラル (`Expression::BigLit`) | Coverage Gap G | 超大整数入力は稀。i64 で 10^18 まで対応可能 |
| D-4 | Interactive NodeKind | P-18, §4.3 | Phase 2 で設計。StructureAST の静的木モデルと根本的に異なる |
| D-5 | Expression 評価意味論の完全形式化 | L-3 | 評価関数の仕様策定が必要。実装フェーズで段階的に定義 |
| D-6 | 依存グラフ構築ルールの完全形式化 | Sample Gen 3.4 | ヒューリスティック（lhs 先行）で多くのケースは対応可能 |
| D-7 | Repeat スコープ内制約の適用範囲の形式化 | Sample Gen 3.6 | 暗黙規約（各イテレーションに独立適用）で実装を開始し、必要に応じて型拡張 |
| D-8 | Repeat.count の Expression 対応 | Coverage Gap A | 暫定対応として暗黙 Scalar 導入で回避可能。実装時に再評価 |
| D-9 | ProjectedNodeKind のドメイン固有分類 | L-2 | annotation tag の導入は Phase 2 で検討 |
...
| 版 | 日付 | 変更内容 |
|----|------|----------|
| Sprint 2 (初版) | — | DomainModelAgent による初期設計 |
| Rev.1 (Sprint 3 Synthesis) | — | S-1: NodeKind から型・separator 除去、ConstraintAST に一元化。S-2: ConstraintId 導入、ConstraintSet 再設計。S-3: Builder 層分離の方針確定（projection-operation.md 側で定義）。M-1: StructureAst を Arena (Vec) ベースに変更。M-4: Tuple.elements を Vec\<NodeId\> に変更。M-5/Gap B: Choice NodeKind 追加。L-1: Hole.hole_id 除去。L-4: Hole への制約付与を許可。Gap F/SG 3.1: CharSet, StringLength, RenderHint 制約追加。M-3: 生成可能保証に制約相互作用条件を追加、3段階保証レベル導入。 |
