...
| **前提条件** | ① target が存在する ② target が制約を受け入れられるノード種別である ③ 制約が既存の制約と矛盾しない |
| **成功時の状態変化** | ConstraintAST に新しい制約ノードが追加される。新しい NodeId が割り当てられる |
| **失敗理由** | `NodeNotFound` / `ConstraintViolation` — 既存制約と矛盾（例: `1 ≤ N ≤ 100` が既にあるのに `N ≤ 50` を追加すると上界が競合） / `InvalidOperation` — hole に制約を付けようとした場合 |
...
| 項目 | 内容 |
|------|------|
| **入力パラメータ** | `constraint_id: NodeId` (制約ノードの ID) |
| **前提条件** | ① constraint_id が制約ノードとして存在する |
| **成功時の状態変化** | ConstraintAST から該当制約が除去される |
| **失敗理由** | `NodeNotFound` — 制約が存在しない / `InvalidOperation` — 削除すると構造的整合性が壊れる制約（例: 配列長制約 `len(A) = N` は EdgeList の構造と不可分） |

**具体例:** ABC300-A で「C_i は相異なる」制約を除去する。

```rust
Action::RemoveConstraint {
    constraint_id: NodeId(20), // Distinct(C) の制約ノード
}
...
**状況:** Hole に対して AddConstraint しようとした。
...
// → Err(OperationError::InvalidOperation {
//     action: "AddConstraint".into(),
//     reason: "Cannot add constraint to a Hole. Fill the hole first.".into(),
// })
```
...
pub trait ProjectionQueryAPI: ProjectionAPI {
    ...
    /// 指定 Action を適用した場合の結果をプレビューする（dry-run）。
    /// 実際の状態変更は行わない。
    fn preview_action(&self, action: &Action) -> Result<PreviewResult, OperationError>;
...
Step 4: agent は preview_action で dry-run
...
pub struct ApplyResult {
    ...
    /// 影響を受けた制約の ID 一覧。
    pub affected_constraints: Vec<NodeId>,
}
```
